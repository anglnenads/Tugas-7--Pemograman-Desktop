...
Public Class frmUtama
    ...
    Private Function cekLangganan(ByVal nomorPlat As String) As Boolean
        Dim berlangganan As Boolean = False
        Dim sql As String = "SELECT COUNT(*) FROM " & tbllangganan & "WHERE no_plat= '" & txtNoPlat.Text & "'"
           ...
           "' and waktu_keluar is null and DATE(`waktu_masuk`) = CURDATE()"
            ...
            If hasil1.HasRows Then 'kalau sudah ada, update jadi keluar
                If MessageBox.Show("Keluar dari Parkir?", "Keluar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                    ...
                    'untuk mengecek pengguna langganan
                    Dim berlangganan As Boolean = cekLangganan(txtNoPlat.Text.ToUpper)
                    If berlangganan Then
                        harga = 0
                        'jika tidak ada dalam daftar langganan maka akan memunculkan harga
                    Else
                        If jns = "Motor" Then
                            harga = 2000
